...
ddl_marts = {
    ...
    "insert_dm_sales": """
        INSERT INTO dm_sales (order_id, order_date, user_id, user_name, payment_type, shipper_name, 
                              order_price, order_discount, voucher_name, order_total, product_id,
                              product_name, product_category_id, product_category_name, user_region)
        SELECT 
            fo.order_id, fo.order_date, fo.user_id, 
            du.user_first_name || ' ' || du.user_last_name AS user_name, 
            dp.payment_name AS payment_type, 
            ds.shipper_name, 
            fo.order_price, fo.order_discount, 
            COALESCE(dv.voucher_name, '') AS voucher_name, 
            fo.order_total,
            foi.product_id, 
            di.product_name, 
            di.product_category_id, 
            dc.product_category_name, 
            du.user_address AS user_region
        FROM 
            fact_orders fo
        INNER JOIN 
            dim_user du ON fo.user_id = du.user_id
        INNER JOIN 
            dim_payment dp ON fo.payment_id = dp.payment_id
        INNER JOIN 
            dim_shipper ds ON fo.shipper_id = ds.shipper_id
        LEFT JOIN 
            dim_voucher dv ON fo.voucher_id = dv.voucher_id  -- Assuming vouchers can be NULL, using LEFT JOIN
        INNER JOIN 
            fact_orders_items foi ON fo.order_id = foi.order_id
        INNER JOIN 
